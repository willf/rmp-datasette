...
rmp_accident_history_csv = r"C:\MS Data Science - WMU\EDGI\rmp\rmp_accident_history.csv"
output_csv = r"C:\MS Data Science - WMU\EDGI\rmp\rmp_accident_history_detailed.csv"
...
def parse_accident_history(section_6_text):
    ...
    for block in accident_blocks:
        ...
        # Chemicals
        quantity_match = re.search(r"Quantity Released \(lbs\):\s*(\d+)", block_text, re.IGNORECASE)
        accident["Quantity Released (lbs)"] = quantity_match.group(1) if quantity_match else None
        percent_match = re.search(r"Percent Weight:\s*(\d+)", block_text, re.IGNORECASE)
        accident["Percent Weight"] = percent_match.group(1) if percent_match else None
        chem_name_match = re.search(r"Chemical Name:\s*(.+)", block_text, re.IGNORECASE)
        accident["Chemical Name"] = chem_name_match.group(1).strip() if chem_name_match else None
        cas_match = re.search(r"CAS Number:\s*(\d{2,7}-\d{2}-\d)", block_text, re.IGNORECASE)
        accident["CAS Number"] = cas_match.group(1) if cas_match else None
        flammable_match = re.search(r"Flammable/Toxic:\s*(Flammable|Toxic)", block_text, re.IGNORECASE)
        accident["Flammable/Toxic"] = flammable_match.group(1) if flammable_match else None
        ...
        try:
            with fitz.open(pdf_file) as doc:
                ...
                if accident_data["Has Accident"] == "No":
                    ...
                    log_file.write(f"Unexpected: {pdf_file.name} has no accidents at {time.ctime()}\n")
                else:
